...
Imagen::Imagen()
        : id("0"), nombre("Sin_nombre"), tam(0), fecha(1, 1, 1980), pos(), etiquetada(), likes(0) {}

Imagen::Imagen(const string &id, const string &nombre, unsigned int tam, unsigned int likes, const Fecha &fecha, const UTM &pos, const deque<Etiqueta *> &etiquetas)
        : id(id), nombre(nombre), tam(tam), likes(likes), fecha(fecha), pos(pos), etiquetada(etiquetas) {}

Imagen::Imagen(const Imagen &orig) : id(orig.id), nombre(orig.nombre), tam(orig.tam), likes(orig.likes),
    fecha(orig.fecha), pos(orig.pos), etiquetada(orig.etiquetada) {}

string Imagen::toString(const bool modo) {
    string aux = id + ":";
    aux += nombre + ":";
    if(!etiquetada.empty() && modo){
        for(Etiqueta* etq : etiquetada)
            if(etq)
                aux += etq->getNombre() + ",";
    }
    aux += fecha.cadena() + ":" + std::to_string(tam) + " bytes :" + std::to_string(getX()) + "," + std::to_string(getY())  + ": " + to_string(likes) + " likes";
    return aux;
}
...
vector<string> Imagen::getEtiquetas() const {
...
const UTM &Imagen::getPos() const {
    return pos;
}

void Imagen::setPos(const UTM &pos) {
    Imagen::pos = pos;
}

const float Imagen::getX() const{
    return pos.getLongitud();
}
const float Imagen::getY() const{
    return pos.getLatitud();
}