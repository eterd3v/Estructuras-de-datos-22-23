...
public:
     ...
     * @param [in] tamlog es el tamaño lógico que se busca del vector
     */
    explicit VDinamico(unsigned int tamlog);

    /**
     * @brief Constructor copia de la clase VDinamico
     * @param [in] origen es una instancia de VDinamico que va a ser copiada para el nuevo objeto
     */
    VDinamico(const VDinamico<T> &origen);

    /**
     * @brief Constructor copia parcial de la clase VDinamico
     * @param [in] origen es un objeto de tipo VDinamico<T> ya instanciado que va a ser parcialmente copiado
     * @param [in] posicionInicial es la posición inicial desde la cual se van a copiar los datos de origen. Va desde 0 hasta TAMAÑO - 1
     * @param [in] numElementos es la cantidad de elementos que se van a copiar para la creación del nuevo vector.
     */
     ...
     * @param [in] orig es la instancia de VDinamico que se va a usar para la asignación
    ...
    /**
     * @brief Operador de igualdad de la clase VDinamico
     * @param [in] orig Es el VDinamico con el que se va a comparar
     * @return true si ambos vectores son iguales. false en cualquier otro caso.ç
     */
    bool operator==(const VDinamico &orig) const;

    /**
     * @brief Operador de lectura y escritura
     * @param [in] posicion es la posición a la que se desea acceder. El rango es desde 1 hasta tamlog(), ambos incluidos.
     ...
     * @param [in] dato del mismo tipo con el que instanció la plantilla del vector que va a ser introducido.
     * @param [in] pos indica la posición donde se desea insertar el dato. Inserta al final por defecto.
     */
    void insertar(const T& dato, unsigned int pos = UINT_MAX);

    /**
     * @brief Método para borrar elementos del vector dinámico
     * @param [in] pos es la posición del vector a borrar. Sólo permite desde 1 hasta tamlog()
     * @return dato de tipo T que se encuentra en la posición a borrar
     */
    T borrar(unsigned int pos = UINT_MAX);

    /**
     * @brief Método para borrar todos los elementos del vector.
     */
    void borrarTodo();
     ...
     * @param [in] dato del mismo tipo con el que se instanció la plantilla del vector para ser buscado.
    ...
    /**
     * @brief Función para tomar el primer valor del vector
     * @return nullptr si el vector está vacío. Puntero con el primer dato en cualquier otro caso
     */
    T* &inicio();

    /**
     * @brief Función para tomar el último valor del vector
     * @return nullptr si el vector está vacío. Puntero con el primer dato en cualquier otro caso
     */
    T* &fin();
...
unsigned int VDinamico<T>::potenciaSup(unsigned int num){
    if(num < 1)
        throw invalid_argument("[VDinamico<T>::potenciaSup] No se puede obtener la potencia del parametro num.");
...
void VDinamico<T>::alojarDoble(){
    T* vAux = new T[tamLogico];         //Copio los elementos del vector auxiliar al mio
    for (int i = 0; i < tamLogico; ++i)
        vAux[i] = vectorDin[i];
    delete[] vectorDin;                 //Lo borro
    vectorDin = new T[tamFisico *= 2];  //Asigno nuevo espacio y volvemos a meter los elementos
    for (int i = 0; i < tamLogico; ++i)
        vectorDin[i] = vAux[i];
}
...
VDinamico<T>::VDinamico(const VDinamico<T> &origen, unsigned int posicionInicial, unsigned int numElementos){
    if(numElementos + posicionInicial > origen.tamLogico) //Aquí tiene más sentido que no se exceda el tamaño lógico y no el físico por ser un vector dinámico
        throw invalid_argument("[VDinamico<T>::VDinamico] El número de elementos a copiar exceden a los que existen en VDinamico<T> &origen.");
    if(numElementos == 0)
        throw invalid_argument("[VDinamico<T>::VDinamico] No se va a copiar ningún elemento");
...
VDinamico<T> &VDinamico<T>::operator=(const VDinamico<T> &orig) {
    ...
    delete vectorDin;
    vectorDin = new T[orig.tamFisico];
    for (int i = 0; i < orig.tamLogico; ++i)
        vectorDin[i] = orig.vectorDin[i];
    this->tamLogico = orig.tamLogico;
    this->tamFisico = orig.tamFisico;

    return *this;
}

template<class T>
bool VDinamico<T>::operator==(const VDinamico<T> &orig) const {
    if(this->tamLogico == orig.tamLogico){
        for(unsigned i = 0; i < this->tamLogico; ++i)
            if(-1 == orig.busquedaBin(this->vectorDin[i]))
                return false;
        return true;
    }
    return false;
...
T& VDinamico<T>::operator[](const unsigned int posicion){
    if(!(1 <= posicion && posicion <= tamLogico))
        throw invalid_argument("[VDinamico<T>::operator[]] Se ha intentado acceder a una posición invalida en el vector");
...
void VDinamico<T>::insertar(const T& dato, unsigned int pos){
    if(tamLogico < pos && pos < UINT_MAX)
        throw length_error("[VDinamico<T>::insertar] No se puede insertar en esa posición ya que no está en el vector o sus límites.");
    ...
    switch (pos) {
        case UINT_MAX:
            ...
            break;
        case 0:
            throw invalid_argument("[VDinamico<T>::insertar] La posición 0 no es válida para insertar un dato. Sólo entre 1 <= sig <= TAM");
        default:
...
T VDinamico<T>::borrar(unsigned int pos){
    if(tamLogico < pos && pos < UINT_MAX)
        throw invalid_argument("[VDinamico<T>::borrar] No se puede borrar en esa posición ya que no está en el vector o sus límites.");
    if(tamLogico == 0)
        throw invalid_argument("[VDinamico<T>::borrar] No hay más elementos en el vector. No se puede borrar más.");
    if(pos == 0)
        throw invalid_argument("[VDinamico<T>::borrar] La posición 0 no es válida para borrar un dato. Sólo entre 1 <= sig <= TAM");

    if(tamLogico * 3 <= tamFisico)         //Esperar a 1/3 del tamaño máximo para disminuir el tamaño fisico del vector
        alojarMitad();

    if(pos == UINT_MAX)                    //si sig == UINT_MAX entonces se borra por el final
        pos = tamLogico;
...
template<class T>
void VDinamico<T>::borrarTodo() {
    delete vectorDin;
    vectorDin = new T[tamFisico=1];
    tamLogico = 0;
}
...
template<class T>
int VDinamico<T>::busquedaBin(const T &dato) const{
    ...
    while(izquierda <= derecha){ //Bucle hasta encontrar el dato.
        ...
        (dato < vectorDin[pos]) ? (derecha = pos - (izquierda+derecha)%2) : (izquierda = pos + (izquierda+derecha)%2); // Solución => sig +- (izq+der)%2 para tb salir del bucle
    }
