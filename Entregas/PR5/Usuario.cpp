...
vector<Imagen *> Usuario::getImagenesEtiqueta(const string &nombreEti) {
    ibAsociado();
    vector<Imagen*> aux;
    vector<Usuario*> usrs = ib->buscarUsuarioEtiq(nombreEti);
    for (Usuario* u : usrs){
        if(u){
            vector<Imagen*> imgs = u->buscarEtiq(nombreEti);
            for (Imagen* i : imgs) {
                if(i)
                    aux.push_back(i);
            }
...
void Usuario::meGustaImagen(Imagen *&img) {
    ...
    Imagen* aux = this->buscarImagen(img->getId());
    if(aux)
...
void Usuario::likeAutomatico(const string &nombreEtiq) {
    ...
    for (Usuario* u : vUsrEtq){ //Pasamos por todos los usuarios del sistema
        if(u){
            vector<Imagen*> vImgUsr = u->getImagenesEtiqueta(nombreEtiq);
