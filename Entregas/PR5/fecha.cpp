...
Fecha::Fecha() {
    time_t tiempoActual;
    struct tm * fechaActual; //He tenido que cambiar esta línea de código por problemas con la nueva versión de Clion

    time(&tiempoActual); // Obtener hora actual

#if (defined(WIN32) || defined(_WIN32) || defined(__WIN32__))
     fechaActual = localtime(&tiempoActual); //He tenido que cambiar esta línea de código por problemas con la nueva versión de Clion
#else
    localtime_r(&tiempoActual, &fechaActual); // POSIX
#endif

    leerTiempo(*fechaActual); //He tenido que cambiar esta línea de código por problemas con la nueva versión de Clion
}
...
bool Fecha::operator==(const Fecha &orig) const {
    return dia == orig.dia &&
           mes == orig.mes &&
           anio == orig.anio &&
           hora == orig.hora &&
